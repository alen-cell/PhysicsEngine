...
{
  /*  private static FluidParticles _Instance;
    public static FluidParticles Instance
    {
        get
        {
            if (_Instance == null)
            {
                _Instance = new FluidParticles();
  ...
  */
   ...
   // public ComputeBuffer pressures { get; private set; } 
    ...
    private string m_fluidDrawingTag = "FluidInstanceDraw";
    private ProfilingSampler m_particleDrawProfile;



    public void ExecuteDraw(CommandBuffer cmd ,ScriptableRenderContext context ,ref RenderingData renderingData, Material overridematerial,Mesh mesh)
    {

        if (m_argsBuffer == null)
            CreateArgBuffer(mesh.GetIndexCount(0));
        if (m_fluidDrawingTag == null)
        {
            m_particleDrawProfile = new ProfilingSampler(m_fluidDrawingTag);
        }
       // CommandBuffer cmd = CommandBufferPool.Get(m_fluidDrawingTag);
        using (new ProfilingScope(cmd, m_particleDrawProfile))
        {
            overridematerial.SetBuffer("_positionsBuffer", this.positions);

            overridematerial.SetFloat("radius", Radius);
            cmd.DrawMeshInstancedIndirect(mesh, 0, overridematerial, 0, m_argsBuffer,0);
