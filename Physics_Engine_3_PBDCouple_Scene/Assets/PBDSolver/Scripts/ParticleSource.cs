...
public class ParticleSource
{
    ...
    public ParticleSource(float spacing, Bounds bounds, Bounds exclusion, bool enableMeshCollider)
    {
        Bounds = bounds;
        Exclusion = new List<Bounds>();
        Exclusion.Add(exclusion);
        // Positions = new List<Vector3>();
        CreateBoundaryParticles(spacing);
    ...
    private void CreateBoundaryParticles(float spacing)
    {
        float HalfSpacing = spacing * 0.5f;
        int numX = (int)((Bounds.size.x + HalfSpacing) / spacing);
        int numY = (int)((Bounds.size.y + HalfSpacing) / spacing);
        ...
        foreach (var collider in MeshToParticleColliders.Colliders)
        {
            var mesh = collider.GetComponent<MeshFilter>().mesh;
            
            // Positions = new List<Vector3>();
            for (int i = 0; i < collider.VertexCount - 1; i++)
            {

                Positions.Add(collider.TRS.MultiplyPoint3x4(mesh.vertices[i]));
              //  Vector3 position = collider.Positions[i];
