...
public class RenderVolume : IDisposable
{
    ...
    public RenderVolume(Bounds bounds, float pixelSize)
    {
        ...
        m_shader = Resources.Load("RenderVolume") as ComputeShader;
    }
    ...
    /// </summary>
    public void FillVolume(FluidParticles fluidbody, GridHash grid, SmoothingKernel kernel)
    {
        ...
        m_shader.SetFloat("Density", fluidbody.Density0);
        m_shader.SetInt("NumParticles", fluidbody.NumParticles);
        m_shader.SetFloat("ParticleVolume", fluidbody.ParticleVolume);
        m_shader.SetBuffer(computeKernel, "IndexMap", grid.IndexMap);
        m_shader.SetBuffer(computeKernel, "Table", grid.Table);
        m_shader.SetBuffer(computeKernel, "Positions", fluidbody.positions);
        m_shader.SetBuffer(computeKernel, "Densities", fluidbody.densities);
